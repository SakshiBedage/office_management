...
function addBoardCard(e) {
  ...
  if (title != "") {
    ...
    left.innerHTML =
      '<img src="../../../assets/img/svg/watch.svg" alt="" class="svg mr-1">';
...
function addList(e) {
  ...
  if (e.keyCode === 13) {
    ...
    if ($(this).val()) {
      ...
      setAttributes(boardHeader, {
        class:
          "board-header d-flex justify-content-between align-items-center mb-10",
      });
      ...
      setAttributes(dropdownIcon, {
        class: "menu-icon justify-content-center style--two mr-0",
      });
      ...
      setAttributes(boardCompose, {
        class:
          "board-composer flex-column d-flex align-items-center justify-content-center",
      });

      var addAnotherCard = d.createElement("a");
      setAttributes(addAnotherCard, { href: "#", class: "add-another-card" });
      addAnotherCard.innerHTML =
      ...
      setAttributes(textarea, {
        ...
        class: "theme-input-style style--five",
        value: "title",
      });
      form.append(textarea);

      var dateActionWrap = d.createElement("div");
      setAttributes(dateActionWrap, {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Check if the element with class "board-wrapper" exists
  if (slider) {
    // Mousedown
    ...
    console.error("Element with class 'board-wrapper' not found.");
  }
});
...
/*============================================
    ** Project Main Title edit Option
    ==============================================*/
var titleTextarea = $("#projectTaskDetails").find(
  ".project-main-title textarea"
);

$("#projectTaskDetails")
  .find(".project-main-title h4")
  .on("click", function (e) {
    e.stopPropagation();
    $(
      "#projectMemberModal, #projectLabelModal, #projectChecklistModal, #projectMoveModal, #projectCopyModal, #projectShareModal, .checklist-add-control-wrap"
    ).hide();
...
projectDes.find(".description").on("click", function (e) {
  ...
  $(this)
    .addClass("edit")
    .siblings(".des-edit-actions")
    .show()
    .children("textarea")
    .text(desText.text())
    .css("height", desText.height() + 8 + "px");
});

projectDes.find(".des-edit-controls .des-save").on("click", function (e) {
  var textareaVal = $(this)
    .parents(".des-edit-controls")
    .siblings("textarea")
    .val();
  $(this)
    .parents(".des-edit-actions")
    .siblings(".description")
    .text(textareaVal);
});

projectDes
  .find(".des-edit-controls .icon-close, .des-edit-controls .des-save")
  .on("click", function (e) {
    $(this)
      .parents(".des-edit-actions")
      .siblings(".description")
...
commentContent.find(".comment-action-edit").on("click", function (e) {
  ...
  var commentText = $(this)
    .parents(".comment-action-links")
    .siblings(".comment-edit")
  ...
  $(this)
    ...
    .css("height", commentText.height() + 5 + "px");
});
...
commentContent
  .find(".comment-edit .icon-close, .comment-edit .comment-save")
  .on("click", function (e) {
    $(this)
      .parents(".comment-edit")
      .removeClass("mb-5")
      .children(".edit-actions")
      .hide();
  });
...
$(".board-labels-list, .board-labels-select-color")
  .find(".label-item .label")
  .on("mouseenter", function (e) {
...
$(".board-labels-select-color, .edit-labels-no-color")
  .find(".label-item .label")
  .on("click", function (e) {
    $(".board-labels-select-color, .edit-labels-no-color")
      .find(".label-item")
...
$(".change-card-btns .change-card-due-date, #default-date2").on(
  "click",
  function (e) {
    e.preventDefault();
...
function addNewLabel() {
  ...
  var labelId = $(".board-labels-select-color, .edit-labels-no-color")
    .find(".label-item.active > .label")
    .attr("id");
  var labelClass = $(".board-labels-select-color, .edit-labels-no-color")
    .find(".label-item.active > .label")
    .attr("class");
...
// Member Add btn
$(".member .btn-circle, .change-card-btns .change-card-member").on(
  "click",
  function (e) {
    e.stopPropagation();
    $(
...
$(document).on(
  "click",
  "#projectTaskDetails .add-label, #projectTaskDetails .change-card-label",
  function (e) {
    e.stopPropagation();
...
$(document).on(
  "click",
  "#projectTaskDetails .change-card-checklist",
  function (e) {
    e.stopPropagation();
    let coords = this.getBoundingClientRect();
    $(
...
$(document).on("click", "#projectTaskDetails .change-card-move", function (e) {
  ...
  if (isIE) {
    projectMoveModal.css({
      left:
        e.pageX - e.target.clientWidth + (window.scrollX ? window.scrollX : 0),
      top:
        e.pageY + e.target.clientHeight + (window.scrollY ? window.scrollY : 0),
    });
  }
});
...
// stop propagation
$(
  "#projectMemberModal, #projectLabelModal, #projectChecklistModal, #projectMoveModal, #projectCopyModal, #projectShareModal, #projectTaskDetails .checklist-new-item"
).on("click", function (e) {
  $(".dropdown-menu.checkbox2").removeClass("show");
  e.stopPropagation();
});
...
// Popup Close
$(
  "#projectMemberModal .modal-close, #projectMoveModal .modal-close, #projectCopyModal .modal-close, #projectLabelModal .modal-close, #projectShareModal .modal-close, #projectChecklistModal .modal-close, #projectTaskDetails .add-item-btn"
).on("click", function () {
  $(
    "#projectMemberModal, #projectLabelModal, #projectShareModal, #projectChecklistModal, #projectMoveModal, #projectCopyModal, .checklist-add-control-wrap"
  ).hide();
  $(".dropdown-menu.checkbox2").removeClass("show");
});
...
function addChecklistEle(ele, checklistItemText) {
  ...
  ele
    .parents(".checklist-new-item")
    .siblings(".checklist-items")
    .append(checklistItemWrap);

  var posRelative = d.createElement("div");
  posRelative.className =
    "checklist-item-details position-relative d-flex justify-content-between";
...
function addChecklistItem(e) {
  ...
  var checklistItemText = $(this)
    .parents(".checklist-add-controls")
    .siblings("textarea");
...
function addChecklist(e) {
  e.preventDefault();
  $(
    "#projectMemberModal, #projectLabelModal, #projectChecklistModal, #projectMoveModal, #projectCopyModal, .checklist-add-control-wrap"
  ).hide();
  ...
  if (inputValue.val() != "") {
    ...
    checklistAddControls.className =
      "checklist-add-controls d-flex align-items-center";
    ...
    iconClose.addEventListener("click", function (e) {
      ...
      $(this)
        .parents(".checklist-add-control-wrap")
        .siblings(".add-item-btn")
        .show();
    });
...
$("#projectChecklistModal")
  .find(".add-checklist-btn")
  .on("click", addChecklist);
$("#projectTaskDetails")
  .find(".add-checklist-item")
...
$("#projectTaskDetails")
  .find(".checklist-new-item textarea")
...
$("#projectTaskDetails")
  .find(".checklist-new-item")
  .on("click", ".add-item-btn", function (e) {
    e.preventDefault();
    $(this).hide();
